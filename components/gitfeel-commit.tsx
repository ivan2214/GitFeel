...
export function GitfeelCommit({
	...
	return (
		<div className="space-y-4">
			{/* Main Commit / Fork Card */}
			<div className={`commit-card ${isFork ? "border-purple-500/80 border-l-4" : ""}`}>
				{/* Fork Header (if this is a fork) */}
				{isFork && forkUser && (
					<div className="flex items-center gap-2 p-4 text-muted-foreground text-sm">
						<Repeat2 className="h-4 w-4" />
						<Link className="transition-colors hover:text-primary" href={`/${lang}/dev/${forkUser.id}`}>
							{forkUser.name}
						</Link>
				...
				{isFork && (forkContent || forkTags.length > 0) && (
					<div className="commit-content pb-0">
						{forkContent && (
				...
				{/* Original Commit Content (always displayed, but nested if it's a fork) */}
				<div className={`${isFork ? "mt-4 border-border border-t pt-4" : ""}`}>
					{/* Commit Header */}
												...
												<DialogContent>
													...
													<div className="space-y-4">
														...
														{forkSelectedTags.length > 0 && (
															<div className="space-y-2">
																<Label className="font-medium text-sm">Tags seleccionados para el fork</Label>
																<div className="flex flex-wrap gap-2">
